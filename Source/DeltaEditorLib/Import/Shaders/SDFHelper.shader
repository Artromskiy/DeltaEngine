...
float rectangle(vec2 samplePosition, vec2 halfSize){
    ...
    vec2 distanceToEdge = abs(samplePosition) - halfSize;
...
float roundedRectangle(vec2 samplePosition, float absoluteRound, vec2 halfSize){ 
...
float CalcAlpha(vec2 samplePosition, vec2 size, float radius){
...
float CalcAlphaForIndependentCorners(vec2 samplePosition, vec2 halfSize, vec4 rect2props, vec4 r){

    samplePosition = (samplePosition - .5) * halfSize * 2;

    float r1 = rectangle(samplePosition, halfSize);
                
    vec2 r2Position = rotate(translate(samplePosition, rect2props.xy), .125);
    float r2 = rectangle(r2Position, rect2props.zw);
    
    vec2 circle0Position = translate(samplePosition, vec2(-halfSize.x + r.x, halfSize.y - r.x));
    float c0 = circle(circle0Position, r.x);
    
    vec2 circle1Position = translate(samplePosition, vec2(halfSize.x - r.y, halfSize.y - r.y));
    float c1 = circle(circle1Position, r.y);
    
    vec2 circle2Position = translate(samplePosition, vec2(halfSize.x - r.z, -halfSize.y + r.z));
    float c2 = circle(circle2Position, r.z);
    
    vec2 circle3Position = translate(samplePosition, -halfSize + r.w);
