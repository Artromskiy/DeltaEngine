...
internal class SceneBatcher : GenericBatcher<float4x4, int, int, GpuCameraData>
{
    private const bool ForceWrites = true;

    private readonly Stack<int> _freeIds;

    private GpuArray<float4x4> Trs => _bufferT0;
    ...
    private readonly struct WriteTrs(GpuArray<float4x4> trs, List<int> trsTransferIndices, PooledSet<int> forceWrite) : ISystem
    {
        ...
        {
            var world = IRuntimeContext.Current.SceneManager.CurrentScene._world;
            PointerWriter<float4x4> writer = trs.Writer;
        ...
        private readonly struct TrsWriterWithTransform(WorldContext ctx, PointerWriter<float4x4> trsArray,
        ...
        private readonly struct TrsWriterWithParent(WorldContext ctx, PointerWriter<float4x4> trsArray,
            ThreadLocal<PooledList<int>> transfer, PooledSet<int> forceWrite) :
            IForEachWithEntity<RendId>
        {
            [Imp(Inl)]
