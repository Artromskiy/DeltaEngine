...
internal class TempRenderBatcher : IRenderBatcher
{
    ...
    public GpuArray<float4x4> Transforms { get; private set; }
    ...
    private readonly List<(Render rend, float4x4 matrix)> _tempRenders = [];
    ...
    public TempRenderBatcher(RenderBase renderBase)
    {
        ...
        Transforms = new GpuArray<float4x4>(renderBase.vk, renderBase.deviceQ, 1);
    }
    ...
    public void Draw(Render render, float4x4 matrix) => _tempRenders.Add((render, matrix));
